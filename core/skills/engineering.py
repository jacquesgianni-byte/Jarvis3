...
class EngineeringSkill(Skill):
    ...
    def execute(self, request: str, agent=None) -> Response:
        ...
        Also handles /engineering intelligence commands (Genesis-045).
        """
        # Genesis-045: /engineering command handling
        req_lower = request.strip().lower().rstrip('?!.')
        if req_lower in ('/engineering', 'engineering status', '/engineering status'):
            return self._engineering_status(agent)
        if req_lower in ('/engineering analyse', '/engineering analyze',
                         'engineering analyse', 'engineering analyze'):
            return self._engineering_analyse(agent)
        if req_lower in ('/engineering approve', 'engineering approve'):
            return self._engineering_decide('approve', '', agent)
        if req_lower.startswith('/engineering reject') or req_lower.startswith('engineering reject'):
            reason = request.strip().split(None, 2)[2] if len(request.strip().split()) > 2 else ''
            return self._engineering_decide('reject', reason, agent)
        if req_lower in ('/engineering defer', 'engineering defer'):
            return self._engineering_decide('defer', '', agent)
    ...
    # ------------------------------------------------------------------
    # Genesis-045: Engineering Intelligence command handlers
    # ------------------------------------------------------------------

    def _get_intel_engine(self, agent):
        """Return the EngineeringIntelligenceEngine from the agent, or None."""
        if agent is None:
            return None
        return getattr(agent, 'intel_engine', None)

    def _engineering_status(self, agent) -> Response:
        """Show DRR, active patterns, and pending proposal."""
        engine = self._get_intel_engine(agent)
        if engine is None:
            return Response(success=True, message='Engineering intelligence not available.')
        proposal = engine.get_pending_proposal()
        if proposal is not None:
            return Response(success=True, message=proposal.formatted_for_user())
        # Show status summary with DRR and active patterns
        summary = engine.status_summary()
        # Append active patterns if any
        try:
            patterns = engine._pattern_store.get_active_patterns()
            if patterns:
                plines = ['', 'Active patterns:',
                    *[f'  [{p.category}] {p.display_title} (seen {p.total_occurrences}x)'
                      for p in patterns[:5]]]
                summary += '\n'.join(plines)
        except Exception:
            pass
    ...
    def _engineering_analyse(self, agent) -> Response:
        """Trigger an immediate analysis cycle."""
        engine = self._get_intel_engine(agent)
        if engine is None:
            return Response(success=True, message='Engineering intelligence not available.')
        log_buffer = getattr(agent, 'log_buffer', None)
        if log_buffer is None:
            return Response(success=True, message='Log buffer not available.')
        lines = log_buffer.drain()
        log_buffer.attach()
        proposal = engine.process_session(lines)
        if proposal is not None:
            return Response(success=True, message=proposal.formatted_for_user())
        return Response(success=True, message=engine.status_summary())

    def _engineering_decide(self, decision: str, reason: str, agent) -> Response:
        """Handle approve / reject[reason_code] / defer decisions."""
        engine = self._get_intel_engine(agent)
        if engine is None:
            return Response(success=True, message='Engineering intelligence not available.')
        if decision == 'approve':
            return Response(success=True, message=engine.approve_proposal())
        if decision == 'reject':
            # Sprint-002: pass reason string; engine extracts code from first word
            return Response(success=True, message=engine.reject_proposal(reason))
        if decision == 'defer':
            return Response(success=True, message=engine.defer_proposal())
        return Response(success=True, message='Unknown engineering decision.')
