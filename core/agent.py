...
from core.engineering.intelligence.engine import EngineeringIntelligenceEngine  # Genesis-045



from core.engineering.intelligence.log_buffer import SessionLogBuffer  # Genesis-045
...
class Agent:
    ...
    def __init__(self, ai=None, timeline_repository=None):  # Genesis-047
        ...
        # Genesis-045: Engineering Intelligence



        self.intel_engine = EngineeringIntelligenceEngine(self.knowledge)



        self.log_buffer   = SessionLogBuffer()
        ...
        self._intel_turn_count = 0



        self._intel_cycle_turns = 10  # configurable: turns per analysis cycle
    ...
    def process(
        ...
        if decision is not None and decision.handled:
            ...
            self._check_intel_cycle()  # Genesis-045
    ...
    def _check_intel_cycle(self) -> None:



        """Genesis-045: Trigger engineering analysis every N turns."""



        self._intel_turn_count += 1



        if self._intel_turn_count < self._intel_cycle_turns:
        ...
        try:
            ...
            proposal = self.intel_engine.process_session(lines)
            ...
            if proposal is not None:
                ...
                self.logger.info(
                    ...
                    "[G045] New engineering proposal: %s",
            ...
            self.logger.warning('[G045] Intel cycle failed: %s', e)
        ...
        try:
            ...
            for _cand in _name_candidates:
                ...
                if (_cand not in _STOP_WORDS
                    ...
                    # CFR-006: register ALL candidates (not just first)





                    # so group members like Tom are in EntityRegistry.
    ...
    def _route(self, intent: Intent, request: str, resolution=None) -> Response:
        ...
        # Genesis-045: /engineering commands — must intercept BEFORE

        # EngineeringIntentDetector fast-path so 'analyse' doesn't

        # get routed to CollaborationRunner.

        _req_norm = request.strip().lower()

        # Normalise Android '/ engineering' (space after slash)

        if _req_norm.startswith('/ '):

            _req_norm = '/' + _req_norm[2:]

        _is_eng_cmd = (

            _req_norm in ('/engineering', '/engineering status',

                          '/engineering analyse', '/engineering analyze',

                          '/engineering approve', '/engineering defer')

            or _req_norm.startswith('/engineering reject')

        )

        if _is_eng_cmd:

            _eng_skill = self.skills.get('engineering')

            if _eng_skill:

                # Pass normalised request so skill sees clean command

                self.logger.info('[TURN_TYPE] type=SYSTEM_CMD')

                return _eng_skill.execute(_req_norm, agent=self)

            self.logger.info('[TURN_TYPE] type=SYSTEM_CMD')

            return Response(success=True, message='Engineering intelligence not available.')
        ...
        # Genesis-045: Engineering Intelligence commands

        if req_lower in (

            "/engineering", "engineering status", "/engineering status",

            "/engineering analyse", "/engineering analyze",

            "engineering analyse", "engineering analyze",

            "/engineering approve", "engineering approve",

            "/engineering defer", "engineering defer",

        ) or req_lower.startswith('/engineering reject') or req_lower.startswith('engineering reject'):

            skill = self.skills.get('engineering')

            if skill:

                return skill.execute(request, agent=self)

            return Response(success=True, message='Engineering skill not available.')
        ...
        # CFR-006: Focus change detection moved into FocusSignalStage (pipeline).





        # ConversationRouter evaluates contextual grounding and produces





        # Decision(FOCUS_CHANGE) when authorised. State mutation is below.
        ...
        try:
            ...
            # CFR-006: Focus change authorised by ConversationRouter.





            if conv_decision.decision_type == DecisionType.FOCUS_CHANGE:





                _fc_payload  = conv_decision.payload or {}





                _fc_entity   = _fc_payload.get("focus_candidate", "")





                _fc_is_group = _fc_payload.get("is_group", False)
                ...
                if _fc_entity:
                    ...
                    _fc_obj = self.dialogue_resolver.detect_focus_change(





                        conv_decision.raw_input
                    ...
                    self.dialogue_resolver.apply_focus_change(_fc_obj, self.session)
                    ...
                    if not _fc_is_group:
                        ...
                        self._recent_entities = [_fc_entity.title()]
                    ...
                    try:
                        ...
                        _rl_req = ReverseLookupRequest(members=[_fc_entity])
                    ...
                    _props = self.property_recall.retrieve_all_properties(_fc_entity)
                    ...
                    if _props:
                        ...
                        _profile = self.semantic_recall.recall(_fc_entity, self.knowledge)
                        ...
                        _parts = [
                            ...
                            self.property_recall._format_retrieve_response(_fc_entity, k, v)
                        ...
                        return Response(success=True, message=" ".join(_parts))
                    ...
                    if _fc_is_group:
                        ...
                        return Response(success=True, message=f"Focusing on {_fc_entity}.")





                    return Response(success=True, message=f"Focusing on {_display}.")
