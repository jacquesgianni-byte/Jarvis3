...
class MarkdownRenderer:
    ...
    def render(self, report: GenesisReport) -> str:
        ...
        if report.metadata is not None:
            m = report.metadata
            lines.append("=" * 60)
            lines.append("ENGINEERING REVIEW")
            lines.append("=" * 60)
            lines.append("")
            lines.append(f"Generated by:  {m.worker}")
            lines.append(f"Worker ID:     {m.worker_id}")
            lines.append(f"Worker Ver.:   {m.worker_version}")
            lines.append(f"Planner:       {m.planner}")
            lines.append(f"Coordinator:   {m.coordinator}")
            lines.append(f"Timestamp:     {m.generated_at}")
            lines.append(f"Duration:      {m.duration_ms:.0f} ms")
            lines.append("")
            lines.append("=" * 60)
            lines.append("")
