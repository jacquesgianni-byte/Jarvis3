"""
Jarvis Flask Application Factory

Creates and configures the Flask app with one shared Agent instance.
This is the Jarvis API -- transport layer only. No business logic here.
"""

from __future__ import annotations

import logging
from flask import Flask

logger = logging.getLogger(__name__)


def create_app(agent, system_registry=None, session_registry=None, orchestrator_coordinator=None) -> Flask:
    """
    Create the Flask application.

    Args:
        agent:                    A fully initialised Agent instance.
        system_registry:          Optional SystemRegistry.
        session_registry:         Optional SessionRegistry.
        orchestrator_coordinator: Optional EngineeringCoordinator for
                                  the Genesis-053 approval workflow.

    Returns:
        Configured Flask application.
    """
    app = Flask(__name__)
    app.config["AGENT"]                    = agent
    app.config["SYSTEM_REGISTRY"]          = system_registry
    app.config["SESSION_REGISTRY"]         = session_registry
    app.config["ORCHESTRATOR_COORDINATOR"] = orchestrator_coordinator   # Genesis-053

    # Register routes
    from apps.server.routes import register_routes
    register_routes(app)

    # Genesis-053: register orchestrator blueprint
    from apps.server.orchestrator_routes import orchestrator_bp
    app.register_blueprint(orchestrator_bp)

    logger.info("[SERVER] Flask app created.")
    return app
